...
Options:
  ...
  --admin-password <password>       Default admin password (if omitted, auto generated)
...
if [[ -z "$MAP_KEY" ]]; then
  if [[ -t 0 ]]; then
    read -r -p "Enter TianDiTu API key: " MAP_KEY
  fi
fi

if [[ -z "$MAP_KEY" ]]; then
  echo "[ERROR] TianDiTu API key is required."
  echo "[TIP] Pass with --map-key or set TIANDITU_API_KEY env."
  exit 1
fi
...
echo "[INFO] Step 1/5: setup Python env"
./setup_linux.sh

# shellcheck disable=SC1091
source .venv/bin/activate

echo "[INFO] Step 2/5: save map key"
python manage_map_key.py set --key "$MAP_KEY"
python manage_map_key.py check || true

echo "[INFO] Step 3/5: write runtime env"
cat > .env.webgis <<EOF
WEBGIS_HOST="$HOST"
WEBGIS_PORT="$PORT"
...
echo "[INFO] Step 4/5: start service"
./start_linux.sh

echo "[INFO] Step 5/5: ensure default admin account"
...
if grep -qi microsoft /proc/version 2>/dev/null; then
  echo "[OK] Windows access URL: http://localhost:${PORT}"
fi
